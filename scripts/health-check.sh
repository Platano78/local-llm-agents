...
detect_orchestrator_type() {
    ...
    RESPONSE=$(curl -s --max-time 10 "http://localhost:$port/v1/chat/completions" \
        ...
        -d '{"model":"orchestrator","messages":[{"role":"user","content":"Say hello"}],"max_tokens":50}' 2>/dev/null)
    END=$(date +%s%3N)
...
if check_server "$ORCHESTRATOR_PORT" "Orchestrator"; then
    ...
    ORCH_TYPE=$(detect_orchestrator_type "$ORCHESTRATOR_PORT")
...
if [ "$WORKER_OK" -eq 1 ]; then
    ...
    echo "  cd your-llm-server-directory && ./start-server-code-agents.sh" >&2
    exit 1
fi
